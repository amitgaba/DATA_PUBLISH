sap.ui.define([
	...
	QUnit.test("Should always return a sorter when sorting", function (assert) {
		...
		assert.strictEqual(this.oGroupSortState.sort("VIEW_NAME").length, 1, "The sorting by VIEW_NAME returned a sorter");
	});
	...
	QUnit.test("Should set the grouping to None if the user sorts by VIEW_NAME and there was a grouping before", function (assert) {
		// Arrange
		this.oModel.setProperty("/groupBy", "ID");

		this.oGroupSortState.sort("VIEW_NAME");
